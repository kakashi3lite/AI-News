...
export function renderDigestHtml(digest) {
  return renderHtml(digest);
}

export async function sendDigestEmail(digest) {
  if (!smtpConfigured()) {
    return { skipped: true, reason: 'SMTP not configured', html: renderHtml(digest) };
  }
  const to = digestRecipients();
  if (to.length === 0) {
    return { skipped: true, reason: 'No DIGEST_EMAILS configured', html: renderHtml(digest) };
  }

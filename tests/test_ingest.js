...
const { extractRSSItems } = require('../news/feedParser.js');
const { normalizeRSSArticle } = require('../news/articleNormalizer.js');
const { deduplicateArticles } = require('../news/articleDeduplicator.js');
const { classifyTopicsKeyword } = require('../news/topicClassifier.js');
...
class NewsIngestionTester {
  ...
  async testRSSParsing() {
    ...
    try {
      const parsed = xmlParser.parse(MOCK_RSS_RESPONSE);
      const items = extractRSSItems(parsed);
  ...
  async testArticleNormalization() {
    ...
    try {
      ...
      const normalized = normalizeRSSArticle(mockRSSItem, mockSource);
  ...
  async testTopicClassification() {
    ...
    try {
      const testArticles = [
        { id: '1', title: 'AI and Machine Learning Breakthrough', description: 'New artificial intelligence software revolutionizes tech industry', category: 'general' },
        { id: '2', title: 'Stock Market Analysis', description: 'Financial markets show strong investment opportunities', category: 'general' },
        { id: '3', title: 'Medical Research Discovery', description: 'Scientists discover new vaccine for disease prevention', category: 'general' }
      ];

      const classified = classifyTopicsKeyword(testArticles);
  ...
  async testDeduplication() {
    ...
    try {
      const duplicateArticles = [
        { id: '1', title: 'Breaking News Story', url: 'https://example.com/news1', description: 'Important news story' },
        { id: '2', title: 'Breaking News Story', url: 'https://example.com/news1', description: 'Important news story' },
        { id: '3', title: 'Different News Story', url: 'https://example.com/news2', description: 'Another news story' },
        { id: '4', title: 'Breaking News Story', url: 'https://different.com/news1', description: 'Same title, different domain' }
      ];

      const unique = deduplicateArticles(duplicateArticles);
  ...
  async testCaching() {
    ...
    try {
      ...
      const testArticles = [
        { id: '1', title: 'Test Article', description: 'Test description', url: 'https://example.com/test' }
      ];
  ...
  async testErrorHandling() {
    ...
    try {
      ...
      const invalidSource = { name: 'Invalid Source', url: 'http://192.0.2.1:1/rss', category: 'test' };
      const result = await engine.fetchSingleRSSSource(invalidSource).catch(err => ({ error: err.message }));
  ...
  async testRateLimiting() {
    ...
    try {
      const engine = new NewsIngestionEngine({ cacheDir: this.tempDir, rateLimitDelay: 100 });
  ...
  async testFullIngestionPipeline() {
    ...
    try {
      ...
      const result = await engine.ingestAllSources({ sources: [] });
...
class IngestionBenchmark {
  constructor() {
    this.metrics = { rssParsingTime: [], classificationTime: [], deduplicationTime: [], totalIngestionTime: [] };
  }
  ...
  async runBenchmarks() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      xmlParser.parse(MOCK_RSS_RESPONSE);
      this.metrics.rssParsingTime.push(Date.now() - start);
    }

    const testArticles = Array(100).fill().map((_, i) => ({
      id: i.toString(), title: `Test Article ${i}`, description: 'Technology artificial intelligence software development', category: 'general'
    }));

    for (let i = 0; i < 5; i++) {
      const start = Date.now();
      classifyTopicsKeyword(testArticles);
      this.metrics.classificationTime.push(Date.now() - start);
    }

    for (let i = 0; i < 5; i++) {
      const start = Date.now();
      deduplicateArticles(testArticles);
  ...
  printBenchmarkResults() {
    ...
    Object.entries(this.metrics).forEach(([metric, times]) => {
      if (times.length > 0) {
        ...
        console.log(`${metric}: Average: ${avg.toFixed(2)}ms, Min: ${Math.min(...times)}ms, Max: ${Math.max(...times)}ms`);
      }
    });
  }
}

module.exports = { NewsIngestionTester, IngestionBenchmark, MOCK_RSS_RESPONSE, MOCK_NEWSAPI_RESPONSE };
...
if (require.main === module) {
  ...
  async function runTests() {
    ...
    if (process.argv.includes('--benchmark')) await benchmark.runBenchmarks();
